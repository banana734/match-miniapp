/**
 * 日常反馈页共用逻辑：家长端 / 导师端两个反馈页共用，页面结构完全一样，
 ...
 *   4. 提交前校验 validate()。
 *
 * 与试课反馈的 useTrialFeedbackPage 结构对称，仅字段名由 trial* 改为 class*，
 * 方便读者对照理解。
 *
 * 用法（见 family-daily-feedback.vue）：
 *   const { form, cardId, syncPageFromRoute, handleChoiceGroupChange, validate } =
 *     useDailyFeedbackPage('学生', '的日常反馈')
 */
import { reactive, ref } from 'vue'

/**
 * @param {string} defaultObjectName - 路由没传 name 时兜底的反馈对象名，如「学生」「老师」
 * @param {string} titleSuffix - 标题后缀，默认「的日常反馈」
 */
export const useDailyFeedbackPage = (defaultObjectName = '该对象', titleSuffix = '的日常反馈') => {
  const objectName = ref(defaultObjectName) // 反馈对象名称（学生 / 老师名），从路由参数读
  const cardId = ref('') // 对应卡片的 id，提交反馈时用它定位后端记录
  ...
  const validate = () => {
    if (!form.classDate || !form.classDuration || !form.continueChoice) {
      return false
    }
